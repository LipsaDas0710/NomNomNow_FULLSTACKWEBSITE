...
useEffect(() => {
  ...
  const favoriteData = {
    ...
    address: business.location?.address || "Unknown address",
  ...
  try {
    if (newActiveState) {
      ...
      console.log("📦 Sending favourite data to backend:", favoriteData);
...
// fallback image using random()
       const randomIndex = Math.floor(parseInt(String(business.id)?.slice(-2), 36) % 7) + 1;
